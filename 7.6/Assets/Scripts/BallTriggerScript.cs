...
public class BallTriggerScript : MonoBehaviour {
	...
	void Start () {
		hp = MAX_HP;
		gameObject.GetComponent<Transform> ().localScale = ball.GetComponent<Transform> ().localScale;
		gameObject.AddComponent<CircleCollider2D> ();
		gameObject.GetComponent<CircleCollider2D> ().isTrigger = true;
		gameObject.GetComponent<CircleCollider2D> ().offset = ball.GetComponent<CircleCollider2D> ().offset;
		gameObject.GetComponent<CircleCollider2D> ().radius = 1.7f * ball.GetComponent<CircleCollider2D> ().radius;
	}
