...
public class Ballscripts : MonoBehaviour {
	...
	public void Update() {
		...
		if (Math.Abs (ball_position.position.y - crow_position.position.y) <= 1 && Math.Abs (ball_position.position.x - crow_position.position.x) <= 1) {
			ball_position.position = crow_position.position;
			if (ball_position.position.x < -22) {
				GameFail();
			}
		}
		//
